...
{
    uartSendBytes((uint8_t *)message, strlen(message), 0);
return;
#ifdef DEBUG_SERIAL_USART2
...
void USART3_IT_IRQ(USART_TypeDef *us)
{
  ...
  if (((*SR) & UART_FLAG_RXNE))
  {
    SERIAL_USART_IT_BUFFERTYPE rword = (*DR        publish(_loopbackTopic.c_str(), Sys::micros());
 & 0x01FF;
...
int serial_usart_buffer_count(volatile SERIAL_USART_BUFFER *usart_buf)
{
  ...
  if (count < 0)
    count += SERIAL_USART_BUFFER_SIZE;
  return count;
}
